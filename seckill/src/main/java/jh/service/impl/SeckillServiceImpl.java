...
import jh.dao.SeckillDao;
import jh.dao.SuccessKilledDao;
import jh.dao.cache.RedisDao;
...
public class SeckillServiceImpl implements SeckillService {
    ...
    // md5盐值字符串，用于混淆MD5
    private final String slat = "asdfadgfdagf%^&&^*^%13fd1g65fg#@$$%456dfg";
...
//        Seckill seckill = seckillDao.queryById(seckillId);
    ...
    public SeckillExecution executeSeckill(long seckillId, long userPhone, String md5)
        ...
        // 执行秒杀逻辑：减库存 + 记录购买行为
        // 修改秒杀逻辑：先记录购买行为，再减库存。这样可以避免重复秒杀导致的对数据库的大量访问
        Date nowTime = new Date();
